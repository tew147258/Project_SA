...
type UserQuery struct {
	...
	// eager-loading edges.
	withUserConfirmation *ConfirmationQuery
...
// QueryUserConfirmation chains the current query on the UserConfirmation edge.
func (uq *UserQuery) QueryUserConfirmation() *ConfirmationQuery {
...
//  WithUserConfirmation tells the query-builder to eager-loads the nodes that are connected to
// the "UserConfirmation" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithUserConfirmation(opts ...func(*ConfirmationQuery)) *UserQuery {
	query := &ConfirmationQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserConfirmation = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withUserConfirmation; query != nil {
		...
		query.Where(predicate.Confirmation(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserConfirmationColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_user_confirmation
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_user_confirmation" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_confirmation" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.UserConfirmation = append(node.Edges.UserConfirmation, n)
		}
	}
